...
# Fullscreen++ : Manuel d'utilisation v2.2
=begin
  Description :
    Permet d'avoir un mode plein écran plus intelligent qui utilise un maximum
    de la surface de l'écran tout en gardant le bon ratio d'écran.
    Alt+Entrée utilise toujours l'ancien mode plein écran.
    Pour utiliser ce nouveau mode plein écran appuyez sur F5.
    Vous pouvez aussi appuyez sur F6 pour changer le ratio d'écran, que ce soit
    en mode plein écran ou fenêtré (les ratios des deux modes sont indépendants).
    Le script sauvegarde automatiquement la dernière configuration utilisée dans
    le fichier Game.ini (fullscreen mode & ratios).
    Donc avant de partager votre jeu n'oubliez pas d'y mettre la config que
    vous voulez par défaut, la première fois qu'on lance votre jeu.
    L'ancien mode plein écran est toujours présent mais vous pouvez le désactiver
    en mettant true ligne 9 (au lieu du false).
    Cependant ça désactivera aussi Alt+F4, et si on appuie sur Alt+Entrée en
    même temps ou presque on peut gruger le script malgré tout.
    
  Log Change :
    2.2 :
      - réparé bug sur vx quand F12
    2.1 :
      - le ratio est automatiquement corrigé si hors de portée de notre écran
      - nouvelles fonctions pour mieux manipuler les ratios
      - nouvelles fonctions pour manipuler le mode plein écran par défaut
    2.0 :
  ...
  Fonctions :
    ...
    Graphics.vx_fullscreen?       : pour savoir si on est en mode plein écran de vx.
    Graphics.fullscreen_mode      : passe en mode plein écran.
    Graphics.vx_fullscreen_mode   : passe en mode plein écran de vx.
    Graphics.windowed_mode        : passe en mode fenêtré.
    Graphics.toggle_fullscreen    : passe d'un mode à l'autre.
    Graphics.toggle_vx_fullscreen : passe d'un mode à l'autre avec le plein écran de vx.
    Graphics.toggle_ratio         : passe d'un ratio à l'autre.
    Graphics.ratio                : retourne le ratio actuel.
    Graphics.ratio = n            : configure le ratio avec un nombre.
=end
